...
{
	public class SubleqCompiler32 : ProgramCompiler
	{
		...
		public override void Write(Program program, BinaryWriter writer)
		{
			...
			foreach (ProgramSegment segment in program.Segments)
			{
				...
				else if (segment is IntValue)
					writer.Write((int)((IntValue)segment).Value);
				else if (segment is UnsignedIntValue)
					writer.Write((int)((UnsignedIntValue)segment).Value);
				else if (segment is LongValue)
					writer.Write((int)((LongValue)segment).Value);
				else if (segment is UnsignedLongValue)
					writer.Write((int)((UnsignedLongValue)segment).Value);
				else if (segment is AddressOf)
					writer.Write((int)((AddressOf)segment).Segment.Address.Offset);
			}
		}
		...
		{
			if (segment is StringValue)
				return (uint)(((StringValue)segment).Value.Length * 4);
