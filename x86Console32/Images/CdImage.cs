	...
	public class CdImage
	{
		...
		public void Save(Stream stream)
		{
			...
			using(BinaryWriter writer = new BinaryWriter(memory))
			{
				...
				// Volume ID
				writer.Write(Encoding.ASCII.GetBytes("X86CONSOLE32                    "));
				// Reserved
				...
				// Volume ID
				writer.Write(Encoding.BigEndianUnicode.GetBytes("x86Console32    "));
				// Reserved
				writer.Write(new byte[8]);
				// Logical Block Count
				...
				writer.Write((short)(_programs[0].Length * 4));
				// First Block
				writer.Write((int)0);
				// Reserved
				...
				foreach(var program in _programs.Skip(_bootLoaderCount))
					writer.Write((uint)program.Address.Offset);
